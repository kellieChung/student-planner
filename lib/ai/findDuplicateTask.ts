...
const ANTHROPIC_TOKENS_PER_TASK = 40;
...
function matchByName(
    ...
    const contained = assignments.find((a) => {
        ...
        return (
            ...
            (assignment.includes(task) || task.includes(assignment))
        );
    });
        ...
        response = await getAnthropicClient().messages.create(
            {
                ...
                max_tokens: PREDICT_TOKENS_BASE + ANTHROPIC_TOKENS_PER_TASK * pending.length,
                temperature: 0,
                system: INSTRUCTIONS,
...
export async function findDuplicateTasksForBatch(
    ...
    try {
        const rawResults = isAnthropicEnabled()
            ? await callAnthropic(pending, promptBody)
